...
class LAHeart1(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        self.sample_list = []
        if 'train' in split:
            with open(os.path.join(base_dir, split + '_la.txt'), 'r') as f:
                self.image_list = f.readlines()
        elif split == 'test':
            with open(os.path.join(base_dir, 'test_la.txt'), 'r') as f:
                self.image_list = f.readlines()
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(os.path.join(self._base_dir, "{}px.h5".format(image_name))):
            h5f = h5py.File(os.path.join(self._base_dir, "{}px.h5".format(image_name)), 'r')
            label = h5f['label'][:]
            h5ftrue = h5py.File(os.path.join(self._base_dir, "{}.h5".format(image_name)), 'r')
            truelabel = h5ftrue['label'][:]
            label[:, label.shape[1]//2, :] = truelabel[:, label.shape[1]//2, :]
        else:
            h5f = h5py.File(os.path.join(self._base_dir, "{}.h5".format(image_name)), 'r')
            label = h5f['label'][:]
        image = h5f['image'][:]
...
class LAHeart2(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        if os.path.exists(os.path.join(self._base_dir, "{}py.h5".format(image_name))):
            h5f = h5py.File(os.path.join(self._base_dir, "{}py.h5".format(image_name)), 'r')
            label = h5f['label'][:]
            h5ftrue = h5py.File(os.path.join(self._base_dir, "{}.h5".format(image_name)), 'r')
            truelabel = h5ftrue['label'][:]
