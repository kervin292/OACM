...
def make_patch(image, mask, startposition, endposition):
    """
    make number patch
    ...
    subimage_startposition = startposition - 10
    subimage_endposition = endposition + 10
    if subimage_startposition < 0:
        subimage_startposition = 0
    if subimage_endposition > imagezsrc:
        subimage_endposition = imagezsrc
    imageroi = image[subimage_startposition:subimage_endposition, :, :]
    maskroi = mask[subimage_startposition:subimage_endposition, :, :]
...
volumePath = r"H:\LITS\data_kits\data_kits\lits_train\scan"
segmentationPath = r"H:\LITS\data_kits\data_kits\lits_train\label"
savePath = r"H:\data_lits\data_lits\processed_" + v + "_h5\\"

if not os.path.exists(savePath):
    os.makedirs(savePath)

if __name__ == "__main__":
    fileList = os.listdir(volumePath)
    for f in fileList:
        ct = sitk.ReadImage(os.path.join(volumePath, f))
        lbl = sitk.ReadImage(os.path.join(segmentationPath, "segmentation-" + f[7:]))
        ...
        if CFG.do_mask_cropping:        # ct, lbl
            ...
            startposition, endposition = getRangImageDepth(lbl_array_maskcrop)  # (75, 512, 512), 45, 73
            new_ct_array, new_lbl_array = make_patch(ct_array_maskcrop, lbl_array_maskcrop, startposition=startposition, endposition=endposition)
