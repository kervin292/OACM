...
def adjust_roi_parameters(last_ten_predictions):
    # 将最后十个预测结果中的张量堆叠成一个新的张量
    stacked_tensor = torch.stack([tensor for tensor, _ in last_ten_predictions])

    # 计算平均值
    mean_prediction = torch.mean(stacked_tensor, dim=0)

    # 根据平均值更新固定的ROI参数
    updated_roi_parameters = compute_roi_parameters(mean_prediction)

    return updated_roi_parameters

def compute_roi_parameters(prediction):
    # 计算预测结果中非零值的比例
    nonzero_count = torch.sum(prediction > 0).item()
    total_count = prediction.numel()

    # 计算当前比例
    current_ratio = nonzero_count / total_count

    # 设置一个基础比例，这里可以是一个经验值或初始值
    base_ratio = 2/3

    # 动态调整比例，根据当前比例进行微调
    # 这里假设我们希望 mask_ratio 在一定范围内波动，例如 ±10%
    adjustment_factor = 0.1
    lower_bound = base_ratio * (1 - adjustment_factor)
    upper_bound = base_ratio * (1 + adjustment_factor)

    # 确保 mask_ratio 在允许的范围内
    if current_ratio < lower_bound:
        mask_ratio = lower_bound
    elif current_ratio > upper_bound:
        mask_ratio = upper_bound
    else:
...
def apply_updated_roi_parameters(roi_parameters, args):
    # 将更新后的ROI参数应用到自训练中
    args.mask_ratio = roi_parameters['mask_ratio']
